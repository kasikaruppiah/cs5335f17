...
function [Abar, Bbar, QTbar, Qbar, x0bar] = getSyntheticDynamics(A, B, QT, Q, R, x0)
    Abar = [A B; zeros(size(B, 2), size(A, 1)) eye(size(B, 2), size(B, 2))];
    Bbar = [B; eye(size(B, 2), size(B, 2))];
    QTbar = [QT zeros(size(QT, 1), size(R, 2)); zeros(size(R, 1), size(QT, 2) + size(R, 2))];
    Qbar = [Q zeros(size(Q, 1), size(R, 2)); zeros(size(R, 1), size(Q, 2)) R];
    x0bar = [x0; zeros(size(R, 1), 1)];
end
